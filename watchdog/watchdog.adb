...
procedure PeriodicTasks_Priority is
   ...
   flag: Integer:= 0;	--overload detection variable	
   Start : Time; -- Start Time of the System
	Calibrator: constant Integer := 620; -- Calibration for correct timing
   ...
   task type overload_detection is 
   	pragma Priority(1);
   end;
   	task watch_dog is
   	entry signal;
   ...
   task body overload_detection is
   		begin
   			loop
   				delay 1.2;
   				watch_dog.signal;
   			end loop;
   		end overload_detection;
   ...
   task body watch_dog is
   	...
   	begin
   		loop
   			select
   				accept signal do
   					Put_Line("watchdog timer is reloaded");
   					reload:= 1200;
   				end signal;
   			or
   				delay 0.1;
   				reload:=reload - 100;
   				if reload <= 0 then 
   					Put_Line("WARNING!!! SYSTEM OVERLOAD!!!");
   				end if;
			end select;
   		end loop;
   ...
   Task_1 : T(1, 6, Warm_Up_Time, 300, 100, 300); -- ID: 1
	                                                   -- Priority: 20
                                                      --	Phase: Warm_Up_Time (100)
	                                                   -- Period 2000, 
	                                                   -- Computation Time: 1000 (if correctly calibrated) 
	                                                   -- Relative Deadline: 2000
	Task_2 : T(2, 5, Warm_Up_Time, 400, 100, 400);
	Task_3 : T(3, 4, Warm_Up_Time, 600, 100, 600);
	Task_4 : T(4, 3, Warm_Up_Time, 1200, 200, 1200);
	overloadDetect : overload_detection;
