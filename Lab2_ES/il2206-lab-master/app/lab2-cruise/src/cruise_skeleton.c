...
OS_STK DummyTask_Stack[TASK_STACKSIZE];
...
#define WatchDogTask_PERIOD 1800
#define OverloadDetectionTask_PERIOD 300
#define DummyTask_PERIOD 300
/*
 ...
 * Helper functions
 */
void dummyTask_TimerCallback (void *ptmr, void *callback_arg)
{
 ...
 */
  ...
  {
    IOWR_ALTERA_AVALON_PIO_CLEAR_BITS(DE2_PIO_REDLED18_BASE,0x3FFFC);
...
void dummyTask(void* pdata)
{
  INT8U err,semerror;
  int load_parameter =50;  //this is the callibrator
  int* keyVal_dec_ptr;
  void* msg;
  int keyVal_dec;
  int x=1;
  while(1)
  {  
    msg=OSMboxPend(Mbox_dummy,1,&err);
    if (err == OS_ERR_NONE)
    { 
      keyVal_dec_ptr = (int*) msg;
      keyVal_dec = *keyVal_dec_ptr;
      keyVal_dec = keyVal_dec/16;
    }
    load_parameter=load_parameter+(load_parameter*(.02)*keyVal_dec);
    printf("Load Calibrator is: %d\n", load_parameter);
    int i,j;
    for(i=0; i<load_parameter;i++)
    {
      for(j=0; j<500; j++)
      {
        x++; 
...
void SwitchIO(void* pdata)
{
  ...
  while(1)
  {
    keyVal_hex = IORD_ALTERA_AVALON_PIO_DATA(DE2_PIO_TOGGLES18_BASE);
    keyVal_hex_dummy=keyVal_hex & 0x3F0; // The Switches we are concerned with
    // IOWR_ALTERA_AVALON_PIO_SET_BITS(DE2_PIO_REDLED18_BASE,keyVal_hex_dummy);
    // IOWR_ALTERA_AVALON_PIO_CLEAR_BITS(DE2_PIO_REDLED18_BASE,~keyVal_hex_dummy & 0x3F0);
    keyVal_dec=(int)keyVal_hex_dummy; //This returns a binary no. with all set bits equal to one
    err = OSMboxPostOpt(Mbox_dummy, (void *) &keyVal_dec, OS_POST_OPT_BROADCAST);
...
void StartTask(void* pdata)
{
  ...
  controlTaskTimer=OSTmrCreate(
    ...
    overloadDetectionTaskTimer=OSTmrCreate(
              ...
              &err);  
    dummyTaskTimer=OSTmrCreate(
              10, //delay
              DummyTask_PERIOD,//period
              OS_TMR_OPT_PERIODIC,
              dummyTask_TimerCallback, 
              dummyTaskSem, 
              "dummyTaskTimer",
              &err);  
  /*
  ...
  Mbox_dummy   = OSMboxCreate((void*) 0);
  ...
  err = OSTaskCreateExt(
      dummyTask, // Pointer to task code
      dummyTaskSem,        // Pointer to argument that is
      // passed to task
      (void *)&DummyTask_Stack[TASK_STACKSIZE-1], // Pointer to top
      // of task stack
      DummyTask_PRIO,
      DummyTask_PRIO,
      (void *)&DummyTask_Stack[0],
      TASK_STACKSIZE,
      (void *) 0,
      OS_TASK_OPT_STK_CHK);
