...
procedure ProducerConsumer_Prot is
   ...
   -- Random Delays
   subtype Delay_Interval is Integer range 50..250; --Delay_Interval will range from 50 to 250
   package Random_Delay is new Ada.Numerics.Discrete_Random (Delay_Interval);
   use Random_Delay;
   G : Generator;

   -- ==> Complete code: Use Buffer
   mybuffer: CircularBuffer; --Create buffer Object

   task type Producer; --Producer Task

   task type Consumer; --Consumer Task
   ...
   begin
      ...
      for I in 1..N loop
         ...
         prodCount:=I;
         mybuffer.Put(prodCount);
         
			
         -- Next 'Release' in 50..250ms
         Next := Next + Milliseconds(Random(G));
         delay until Next; --Put task in blocking mode
      end loop;
   end;
